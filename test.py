...
class Block(nn.Module):
	...
	def __init__(self, in_channels, out_channels, stride=1, downsample=None, no_relu=False):
		...
		self.bn1 = nn.SyncBatchNorm(out_channels, momentum=BATCH_NORM_MOM)
		self.relu = nn.ReLU(inplace=True)
		self.conv2 = conv3x3(out_channels, out_channels)
		self.bn2 = nn.SyncBatchNorm(out_channels, momentum=BATCH_NORM_MOM)
...
class BottleNeck(nn.Module):
	...
	def __init__(self, in_channels, out_channels, stride=1, downsample=None, no_relu=True):
		...
		self.bn3 = nn.SyncBatchNorm(out_channels * self.EXPANSION, momentum=BATCH_NORM_MOM)
...
class DAPPM(nn.Module):
	def __init__(self, in_channels, branch_channels, out_channels):
		...
		self.scale1 = nn.Sequential(
			...
			nn.SyncBatchNorm(in_channels, momentum=BATCH_NORM_MOM),
		...
		self.compression = nn.Sequential(
			nn.SyncBatchNorm(branch_channels * 5, momentum=BATCH_NORM_MOM),
			nn.ReLU(inplace=True),
...
class SegmentationHead(nn.Module):
	def __init__(self, in_channels, inter_channels, out_channels, scale_factor=None):
		...
		self.bn1 = nn.SyncBatchNorm(in_channels, momentum=BATCH_NORM_MOM)
		self.conv1 = nn.Conv2d(in_channels, inter_channels, kernel_size=3, padding=1, bias=False)
		self.bn2 = nn.SyncBatchNorm(inter_channels, momentum=BATCH_NORM_MOM)
		self.relu = nn.ReLU(inplace=True)
...
class DDRNet(nn.Module):
	def __init__(self, block, layers, num_classes=19, channels=64, spp_channels=128, head_channels=128, augment=False):
		...
		self.conv1 = nn.Sequential(
			...
			nn.SyncBatchNorm(channels, momentum=BATCH_NORM_MOM),
			nn.ReLU(inplace=True),
		...
		self.layer1 = self.create_layer(block, channels, channels, layers[0])
		self.layer2 = self.create_layer(block, channels, channels*2, layers[1], stride=2)
		self.layer3 = self.create_layer(block, channels * 2, channels * 4, layers[2], stride=2)
		self.layer4 = self.create_layer(block, channels * 4, channels * 8, layers[3], stride=2)
  
		self.compression3 = nn.Sequential(
			nn.Conv2d(channels * 4, highres_channel, kernel_size=1, bias=False),
			nn.SyncBatchNorm(highres_channel, momentum=BATCH_NORM_MOM),
		)
		...
		self.down3 = nn.Sequential(
			...
			nn.SyncBatchNorm(channels * 4, momentum=BATCH_NORM_MOM),
		)
		...
		self.down4 = nn.Sequential(
			...
			nn.SyncBatchNorm(channels * 8, momentum=BATCH_NORM_MOM),
		)
		...
		self.layer5 = self.create_layer(BottleNeck, channels * 8, channels * 8, 1, stride=2)
	...
	def create_layer(self, block, in_channels, out_channels, blocks, stride=1):
		...
		if stride != 1 or in_channels != out_channels * block.EXPANSION:
			downsample = nn.Sequential(
				...
				nn.SyncBatchNorm(out_channels * block.EXPANSION, momentum=BATCH_NORM_MOM),
			)
...
def DualResNet_imagenet(pretrained=False):
	...
	checkpoint = torch.load('DDRNet23_imagenet.pth', map_location='cpu') 
...
if __name__ == '__main__':
	model = get_seg_model('DDRNet23_imagenet.pth')
	# print(model)
